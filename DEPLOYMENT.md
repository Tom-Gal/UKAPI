...
- PostgreSQL settings with `DB_CONNECTION=pgsql`; use the provider's TLS mode (`require` or `verify-full`) when available.
- Valkey settings with `CACHE_STORE=redis`, a unique `CACHE_PREFIX`, and either `VALKEY_URL` or host/credential values.
- A real `MAIL_MAILER` and verified `MAIL_FROM_ADDRESS`. The local `log` mailer does not deliver verification or password-reset messages.
...
Restart any PHP workers after deploy. If queues are enabled later, also run
`php artisan queue:restart` as part of the release.

## Operational checks

- `/up` is a lightweight liveness endpoint.
- `/ready` verifies PostgreSQL and the configured cache without disclosing dependency details; it returns `503` when either is unavailable.
...
- Back up PostgreSQL before migrations and verify email delivery, Valkey connectivity, CORS, API-key issuance, and the live Companies House credential before widening access.
